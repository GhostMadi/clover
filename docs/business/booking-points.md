...
Как у посещаемости ([attendance.md](attendance.md)): на хабе точки тайл **Чат**.

1. RPC `booking_open_point_chat(point_id)` — создаёт группу `Запись · {имя точки}` при необходимости.
2. Синхронизирует: **хозяин** (admin) + все **active** `booking_staff` с `profile_id` у этого host.
3. Клиент открывает обычный групповой чат.

Invite исполнителя — по-прежнему **DM** ([booking-staff-plan.md](booking-staff-plan.md)); групповой чат — рабочий канал точки.
...
| Hub точки | Inbox / услуги / **команда** / **чат** / аналитика / расписание — все с `pointId` |
