...
Привязка услуги — **только при создании поста**. Смена CTA после публикации **не делаем** (кейс в `docs/business/booking.md`).  
`set_post_booking_service` может остаться для служебного сброса / редких путей; отдельного UI редактирования нет.
