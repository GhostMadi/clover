import Link from "next/link";
import { redirect } from "next/navigation";
import { AdminLogoutButton } from "@/features/admin/components/admin-logout-button";
import { getAdminSessionFromCookies } from "@/lib/admin-auth";

const MODULES = [
  {
    href: "/admin/support",
    title: "Support",
    desc: "Заявки с /support — статус и ответы",
  },
  {
    href: "/admin/analytics",
    title: "Аналитика",
    desc: "DAU, юзеры, теги сервисов, контент",
  },
  {
    href: "/admin/honest",
    title: "Honest Quiz",
    desc: "Прохождения временного теста",
  },
  {
    href: "/app",
    title: "Кабинет",
    desc: "Обычный /app (нужна сессия пользователя)",
  },
] as const;

export default async function AdminHomePage() {
  const email = await getAdminSessionFromCookies();
  if (!email) redirect("/admin");

  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col gap-6 px-5 py-10">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[13px] font-semibold text-brand">Clover Admin</p>
          <h1 className="mt-1 text-[24px] font-bold tracking-tight text-ink">Панель</h1>
          <p className="mt-2 text-[14px] text-muted">{email}</p>
        </div>
        <AdminLogoutButton />
      </header>

      <section className="grid gap-3 sm:grid-cols-2">
        {MODULES.map((mod) => (
          <Link
            key={mod.href}
            href={mod.href}
            className="rounded-[16px] border border-line bg-surface p-4 transition hover:border-brand/40"
          >
            <h3 className="text-[15px] font-bold text-ink">{mod.title}</h3>
            <p className="mt-1 text-[13px] text-muted">{mod.desc}</p>
          </Link>
        ))}
      </section>
    </main>
  );
}
