...
type HostBookingDetailPanelProps = {
  bookingId: string;
  /** После смены статуса / переноса — обновить список слева. */
  onChanged?: () => void;
  /** Компактный вид для правой колонки inbox. */
  embedded?: boolean;
};

/** Тело детали записи хозяина (без shell) — для страницы и master–detail. */
export function HostBookingDetailPanel({
  bookingId,
  onChanged,
  embedded = false,
}: HostBookingDetailPanelProps) {
