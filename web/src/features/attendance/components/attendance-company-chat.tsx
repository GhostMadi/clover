...
function companyConversation(id: string, title: string): ChatConversation {
  return {
    id,
    type: "group",
    title,
...
/** Чат команды компании внутри рабочего стола посещаемости. */
export function AttendanceCompanyChat({
  conversationId,
  onClose,
}: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setConversation(found ?? companyConversation(conversationId, "Команда компании"));
      } catch (e: unknown) {
        if (!cancelled) {
