...
class _RegisterEmailPageState extends State<RegisterEmailPage> {
  ...
  Widget _body(AuthState state, bool isLoading) {
    if (state is AuthPasswordSetupRequired && state.purpose == AuthOtpPurpose.register) {
    ...
    if (state is AuthEmailOtpSent && state.purpose == AuthOtpPurpose.register) {
      return AuthOtpStep(
        email: state.email,
        otpController: _otpController,
        isLoading: isLoading,
        onResend: () => context.read<AuthCubit>().resendRegisterEmailOtp(state.email),
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AuthCubit, AuthState>(
      listenWhen: (prev, curr) =>
          curr is AuthError ||
          (curr is AuthEmailOtpSent && curr.resumedWithoutResend && curr.purpose == AuthOtpPurpose.register),
