-- Online booking: instant confirmation (confirmed + confirmed_at), not pending.
...
comment on function public.create_booking(uuid, uuid, uuid, timestamptz, int, text, boolean) is
  'Client books a slot; status confirmed immediately (free slot = booked).';
