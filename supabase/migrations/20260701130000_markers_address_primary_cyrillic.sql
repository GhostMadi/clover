-- markers: address_text → address_primary + address_cyrillic (как public.locations).
-- address_text удаляем; старые значения переносим в address_cyrillic.

alter table public.markers
  add column if not exists address_primary text null,
  add column if not exists address_cyrillic text null;

-- Backfill из legacy address_text.
do $$
begin
  if exists (
    select 1
    ...
    execute $q$
      ...
      set address_cyrillic = coalesce(address_cyrillic, address_text)
      where address_text is not null
        and char_length(trim(address_text)) > 0
    $q$;

    alter table public.markers drop column address_text;
  end if;
end $$;

comment on column public.markers.address_primary is
  'Основной адрес события (латиница); денормализация из locations.address_primary при location_id.';

comment on column public.markers.address_cyrillic is
  'Адрес кириллицей (необязательно); денормализация из locations.address_cyrillic при location_id.';

-- --------------------------------------------------------------------------- Sync geo + address from locations when location_id set
...
begin
  ...
  select l.latitude, l.longitude, l.address_primary, l.address_cyrillic
    into v_lat, v_lng, v_primary, v_cyrillic
...
comment on function public.get_post_enriched(uuid) is
  'Post detail: post, author, my reaction/saved, my_following_author; marker: address_primary/address_cyrillic.';
