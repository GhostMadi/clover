-- Fix: remote had legacy m2m profile_tag_links(profile_id, tag_id) without profiles.tag_link_id.
-- Recreate tag storage as one link row per profile with tag_ids uuid[].

drop function if exists public.sync_profile_tag_link(uuid[]);
