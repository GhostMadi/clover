-- Enriched post payloads: include profile_filters[] from post_profile_filter_links.

create or replace function public.post_profile_filters_json(p_post_id uuid)
returns jsonb
language sql
stable
security definer
...
as $fn$
  select coalesce(
    jsonb_agg(
      ...
      order by c.sort_order, c.created_at, v.sort_order, v.created_at
    ),
    '[]'::jsonb
  )
  from public.post_profile_filter_links l
  join public.profile_filter_values v on v.id = l.filter_value_id
  join public.profile_filter_categories c on c.id = v.category_id
  where l.post_id = p_post_id;
$fn$;

comment on function public.post_profile_filters_json(uuid) is
  'JSON array [{category_id, category_name, label}] for a post; SECURITY DEFINER.';
