-- Fix 403 on get_post_enriched: security invoker cannot SELECT profile_follows
-- (grants revoked for authenticated/anon). Use security definer is_following_user().
...
comment on function public.get_post_enriched(uuid) is
  'Post detail: post, author, my reaction/saved, my_following_author (via is_following_user).';
