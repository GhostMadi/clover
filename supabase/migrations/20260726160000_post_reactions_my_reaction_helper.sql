-- Надёжное чтение своей реакции в enriched-RPC (как is_following_user):
-- security invoker + JOIN post_reactions может не вернуть строку из-за RLS/grants.

create or replace function public.get_my_post_reaction_kind(p_post_id uuid)
returns text
language sql
stable
security definer
...
comment on function public.get_my_post_reaction_kind(uuid) is
  'Current user reaction kind for post (like|dislike|null); SECURITY DEFINER, RLS off.';

revoke all on function public.get_my_post_reaction_kind(uuid) from public;
grant execute on function public.get_my_post_reaction_kind(uuid) to authenticated, anon;

-- Batch helper: тот же контракт, без зависимости от RLS на post_reactions.
...
comment on function public.get_my_post_reactions(uuid[]) is
  'Batch: current user reactions for post ids; SECURITY DEFINER, RLS off.';
...
drop function if exists public.list_events_feed_enriched_cursor(jsonb);
