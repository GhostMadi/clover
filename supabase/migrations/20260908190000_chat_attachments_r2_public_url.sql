-- Chat attachments on Cloudflare R2: public_url + bucket `r2`.

alter table public.chat_message_attachments
  add column if not exists public_url text;

comment on column public.chat_message_attachments.public_url is
  'Absolute HTTPS URL when bytes live outside Supabase Storage (e.g. R2).';
...
begin
  ...
  loop
    ...
    if bkt = 'chat_media' then
      ...
      -- Keys from get-upload-url: {folder}/{uid}/{uuid}-name
...
comment on function public.send_message_with_attachments(uuid, text, text, uuid, jsonb, uuid) is
  'Insert message + attachments; bucket chat_media (Storage) or r2 (public_url required).';
