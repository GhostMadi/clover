-- Social in-app notifications → also enqueue FCM via push_outbox (likes/comments/follow).
-- Previously only chat/booking/attendance/login wrote to push_outbox; social stayed in-app only.
...
comment on function public.upsert_notification(uuid, uuid, text, text, uuid, uuid, jsonb, uuid) is
  'Upsert in-app notification; on first insert also enqueue social kinds to push_outbox for FCM.';
